...
describe( "dice roll yeilds a result", () => {
  it( "rolls default properly", () => {
    const response = Dice.dice( createDiceInteraction() );
    expect( response ).toBeTruthy();
  });
  it( "rolls with custom size, default count", () => {
    const response = Dice.dice( createDiceInteraction( 50 ) );
    expect( response ).toBeTruthy();
  });
  it( "rolls with custom size and count", () => {
    const response = Dice.dice( createDiceInteraction( 50, 5 ) );
    expect( response ).toBeTruthy();
  });
  ...
  it( "rolls with hex values", () => {
    const response = Dice.dice( createDiceInteraction( "0x5f", "0x2" ) );
    expect( response ).toBeTruthy();
  });
  it( "doesn't break on invalid size", () => {
    const response = Dice.dice( createDiceInteraction( "not a number" ) );
    expect( response ).toBeTruthy();
  });
  it( "doesn't break on invalid count", () => {
    const response = Dice.dice( createDiceInteraction( "5", "not a number" ) );
    expect( response ).toBeTruthy();
  });
  it( "aborts a count that's too high", () => {
    const response = Dice.dice( createDiceInteraction( "5", Number.MAX_SAFE_INTEGER ) );
    expect( response ).toBeTruthy();
  });
